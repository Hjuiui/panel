            ...
            if (!upgradeHeader || upgradeHeader !== 'websocket') {
                ...
                switch (url.pathname) {
                    ...
                    case '/panel':
                        ...
                        if (! await env.hosein.get('proxySettings')) await updateDataset(env);
                    ...
                    case '/login':
                        ...
                        let secretKey = await env.hosein.get('secretKey');
                        const pwd = await env.hosein.get('pwd');
                        if (!pwd) await env.hosein.put('pwd', 'admin');

                        if (!secretKey) {
                            secretKey = generateSecretKey();
                            await env.hosein.put('secretKey', secretKey);
                        }

                        if (request.method === 'POST') {
                            const password = await request.text();
                            const savedPass = await env.hosein.get('pwd');
...
const getNormalConfigs = async (env, hostName, client) => {
    ...
    try {
        proxySettings = await env.hosein.get("proxySettings", {type: 'json'});
    } catch (error) {
        console.log(error);
...
const buildWorkerLessConfig = async (env, client) => {
    ...
    fragConfig.remarks  = '💦 vpsstar Frag - WorkerLess ⭐'
...
const getFragmentConfigs = async (env, hostName, client) => {
    ...
    if (outProxy) {
        ...
        try {
            ...
            await env.hosein.put("proxySettings", JSON.stringify({
                ...proxySettings, 
                outProxy: '',
                outProxyParams: ''}));
        }
    ...
    bestPing.remarks = '💦 vpsstar Frag - Best Ping 💥';
...
const updateDataset = async (env, Settings) => {
    ...
    try {    
        await env.hosein.put("proxySettings", JSON.stringify(proxySettings));          
    } catch (error) {
        console.log(error);
...
const Authenticate = async (request, env) => {
    ...
    try {
        const secretKey = await env.hosein.get('secretKey');
	...
	<body>
		...
		<div class="form-container">
			...
			<div class="table-container">
				<table id="normal-configs-table">
					...
					<tr>
						...
						<td>
                            <button onclick="openQR('https://${hostName}/sub/${userID}#config-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}#config-Normal', false)">
					...
					<tr>
						...
						<td>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=singbox#config-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
						</td>
					</tr>
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('sing-box://import-remote-profile?url=https://${hostName}/sub/${userID}?app=sfa#config-Normal', 'Normal Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/sub/${userID}?app=sfa#config-Normal', false)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
			...
			<div class="table-container">
                <table id="frag-sub-table">
                    ...
                    <tr>
                        ...
                        <td>
                            <button onclick="openQR('https://${hostName}/fragsub/${userID}#config Fragment', 'Fragment Subscription')" style="margin-bottom: 8px;">
                                QR Code&nbsp;<span class="material-symbols-outlined">qr_code</span>
                            </button>
                            <button onclick="copyToClipboard('https://${hostName}/fragsub/${userID}#config Fragment', true)">
                                Copy Sub<span class="material-symbols-outlined">format_list_bulleted</span>
                            </button>
                        </td>
                    </tr>
            ...
            <div class="footer">
                ...
                <a class="link" href="https://github.com/hoseinzxz" target="_blank">Github</a>
...
const renderErrorPage = (message, error, refer) => {
    ...
    <body>
        <div id="error-container">
            <h1>hosein Panel <span style="font-size: smaller;">${panelVersion}</span> </h1>
            <div id="error-message">
                <h2>${message} ${refer 
                    ? 'Please try again or refer to <a href="https://github.com/hoseinzxz"/blob/main/README.md">documents</a>' 
                    : ''}
                </h2>
